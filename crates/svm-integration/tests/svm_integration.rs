...
fn test_svm_mock_executor_basic() {
    ...
    assert_eq!(
        ...
        SvmConfig::default().compute_unit_limit / 2
    );
}
...
fn test_svm_mock_custom_gas() {
    ...
    let executor = MockSvmExecutor;
    let config = SvmConfig::new(100_000, 2, 100, 1000, 1);
...
fn test_svm_compute_pricing() {
    let config = SvmConfig::new(
        200_000, 10, // 10 microlamports per compute unit
        0, 0, 1,
    );

    // At 200k compute units with 10 microlamports each
    // Total cost would be 2,000,000 microlamports (2 SOL)
    let expected_cost = (config.compute_unit_limit as u128) * (config.compute_unit_price as u128);
...
fn test_svm_block_context() {
    let config = SvmConfig::new(
        200_000, 1, 1_000_000,  // Block height
...
fn test_svm_validation_success() {
    ...
    assert!(
        result.is_ok(),
        "Valid non-empty payload should pass validation"
    );
}
...
fn test_svm_logs_are_opaque() {
    ...
    let result = SvmExecutionResult {
        ...
        account_updates: vec![],
        state_root: [0u8; 32],
        logs: vec![vec![0x01, 0x02, 0x03], vec![0x04, 0x05]],
    };
...
fn test_svm_mock_output() {
    ...
    let executor = MockSvmExecutor;
    let result = executor
        .execute(&[0x01], &[0u8; 32], &SvmConfig::default())
        .unwrap();
