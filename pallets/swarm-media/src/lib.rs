...
pub mod pallet {
    ...
    pub type Contributors<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, Contributor, OptionQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_contributor(
            ...
            let id: BoundedVec<u8, MaxStringLength> = account
                .encode()
                .try_into()
                .map_err(|_| Error::<T>::StringTooLong)?;

            let bounded_name: BoundedVec<u8, MaxStringLength> = name
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::StringTooLong)?;

            let bounded_email: BoundedVec<u8, MaxStringLength> =
                email.try_into().map_err(|_| Error::<T>::StringTooLong)?;
        ...
        pub fn submit_repurposing_job(
            ...
            let job_id_raw =
                sp_io::hashing::blake2_256(&[&source_id[..], &title[..]].concat()).to_vec();
            let job_id: BoundedVec<u8, MaxStringLength> = job_id_raw
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::StringTooLong)?;
            ...
            let bounded_target: BoundedVec<u8, MaxStringLength> = target
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::StringTooLong)?;
        ...
        pub fn update_job_status(
            ...
            let bounded_job_id: BoundedVec<u8, MaxStringLength> = job_id
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::StringTooLong)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn get_job(job_id: &Vec<u8>) -> Option<JobStatusRecord> {
            let bounded_job_id: BoundedVec<u8, MaxStringLength> = job_id.clone().try_into().ok()?;
