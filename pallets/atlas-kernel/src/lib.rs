...
pub enum ValidatorStatus {
    ...
    Paused,
    /// Validator has been removed from the set
    Removed,
}

/// Validator information stored in the registry
#[derive(Clone, PartialEq, Eq, Encode, Decode, RuntimeDebug, TypeInfo)]
pub struct ValidatorInfo<AccountId, PublicKey> {
    /// Validator's account ID
    pub account: AccountId,
    /// Validator's public key for cryptographic verification
    pub pub_key: PublicKey,
    /// Validator's current status
    pub status: ValidatorStatus,
}

/// Control actions that can be performed via multi-sig proposals
#[derive(Clone, PartialEq, Eq, Encode, Decode, RuntimeDebug, TypeInfo)]
...
/// Proposal details for multi-sig control actions
#[derive(Clone, PartialEq, Eq, Encode, Decode, RuntimeDebug, TypeInfo)]
pub struct ProposalDetails<AccountId, PublicKey> {
    /// The action being proposed
    pub action: ControlAction,
    /// Account that created the proposal
    pub created_by: AccountId,
    /// Signatures from validators
    pub signatures: Vec<(AccountId, PublicKey)>,
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + pallet_timestamp::Config {
        ...
        /// Maximum length allowed for HTLC preimage secrets.
        #[pallet::constant]
        type MaxHtlcPreimageLength: Get<u32>;
    ...
    /// Type alias for `Comit` parameterized by the pallet's generic types.
    pub type ComitOf<T> = Comit<<T as frame_system::Config>::AccountId, <T as Config>::Balance>;
    ...
    /// HTLC State Storage: Maps HTLC IDs to their on-chain state and metadata
    /// Key: HTLC ID, Value: HtlcState with all lock/claim/refund information
    #[pallet::storage]
    pub type HtlcStates<T: Config> = StorageMap<
        _,
        Blake2_128Concat,
        u64, // HTLC ID
        (
    ...
    /// HTLC Counter: Global counter for generating unique HTLC IDs
    #[pallet::storage]
    pub type HtlcCounter<T: Config> = StorageValue<_, u64, ValueQuery>;

    /// HTLC Proofs: Stores proof records of successful claims for cross-chain relay.
    /// Key: HTLC ID, Value: Proof data (preimage, chain IDs, etc.)
    #[pallet::storage]
    pub type HtlcProofs<T: Config> = StorageMap<
        _,
        Blake2_128Concat,
        u64, // HTLC ID
        (
    ...
    /// Validator Registry: Maps validator account IDs to their validator information
    ...
    /// Proposal Registry: Maps proposal IDs to their proposal details
    ...
    #[pallet::storage]
    pub type ProposalCounter<T: Config> = StorageValue<_, u32, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// HTLC lock was created with hash-time constraints.
        HtlcCreated {
            htlc_id: u64,
        ...
        /// HTLC was claimed with valid preimage.
        HtlcClaimed {
            htlc_id: u64,
            claimer: T::AccountId,
            amount: T::Balance,
        ...
        /// HTLC was refunded after timelock expiry.
        HtlcRefunded {
            htlc_id: u64,
            initiator: T::AccountId,
            amount: T::Balance,
        },
        /// HTLC claim proof was recorded for cross-chain relay.
        HtlcProofRecorded {
            htlc_id: u64,
            from_chain: u32,
            to_chain: u32,
        ...
        /// A validator was registered in the validator registry
        ValidatorRegistered {
            validator: T::AccountId,
            pub_key: Vec<u8>,
        },
        /// A validator's status was updated
        ValidatorStatusUpdated {
            validator: T::AccountId,
            new_status: ValidatorStatus,
        },
        /// A new multi-sig proposal was created
        ProposalCreated {
            proposal_id: u32,
            action: ControlAction,
            creator: T::AccountId,
        },
        /// A validator signed a multi-sig proposal
        ProposalSigned {
            proposal_id: u32,
            signer: T::AccountId,
            signature_count: u32,
    ...
    pub enum Error<T> {
        ...
        HtlcAlreadyRefunded,
        /// HTLC preimage doesn't match secret hash.
        HtlcInvalidPreimage,
        /// HTLC timelock hasn't expired yet.
        HtlcTimelockNotExpired,
        /// Caller is not authorized to claim/refund this HTLC.
        HtlcUnauthorized,
        /// Invalid HTLC amount (must be > 0).
        HtlcInvalidAmount,
        /// HTLC timelock is in the past or too far in the future.
        HtlcInvalidTimelock,
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Register a validator in the validator registry
        /// Only callable by governance (root or collective).
        #[pallet::call_index(13)]
        #[pallet::weight(<T as Config>::WeightInfo::register_validator())]
        pub fn register_validator(
            ...
            ensure!(!pub_key.is_empty(), Error::<T>::EmptySymbol);

            // Check if validator already exists
            ensure!(
                !ValidatorRegistry::<T>::contains_key(&validator),
                Error::<T>::AuthorityAlreadyExists
            );
            ...
            ValidatorRegistry::<T>::insert(&validator, validator_info);

            // Update validator count
            ValidatorCount::<T>::mutate(|count| *count = count.saturating_add(1));

            // Emit event
            Self::deposit_event(Event::ValidatorRegistered { validator, pub_key });

            Ok(())
        }

        /// Update validator status (e.g., pause, resume, remove)
        /// Only callable by governance (root or collective).
        #[pallet::call_index(10)]
        #[pallet::weight(<T as Config>::WeightInfo::update_validator_status())]
        pub fn update_validator_status(
            ...
            ValidatorRegistry::<T>::mutate(&validator, |info| {
        ...
        /// Create a new multi-sig proposal for control actions
        ...
        pub fn create_proposal(origin: OriginFor<T>, action: ControlAction) -> DispatchResult {
            ...
            // Check if caller is a registered validator
                ...
                ValidatorRegistry::<T>::contains_key(&who),
            ...
            let proposal_id = ProposalCounter::<T>::mutate(|id| {
                *id = id.saturating_add(1);
                *id
            });

            // Create proposal
            let proposal = ProposalDetails::<T> {
                action,
                created_by: who.clone(),
                signatures: vec![(who.clone(), vec![])], // Add creator's signature
                created_block: current_block.saturated_into(),
                executed: false,
            };

            // Store proposal
            ProposalRegistry::<T>::insert(proposal_id, proposal);
        ...
        /// Sign an existing multi-sig proposal
        /// Can be called by any validator
        #[pallet::call_index(12)]
        #[pallet::weight(<T as Config>::WeightInfo::sign_proposal())]
        pub fn sign_proposal(
            ...
            // Check if validator already signed
                ...
                !proposal.signatures.iter().any(|(acc, _)| acc == &who),
                Error::<T>::AuthorityAlreadyExists
            );

            // Add signature
            proposal.signatures.push((who.clone(), signature.clone()));

            // Check if proposal has enough signatures for execution
            let _active_validators = ValidatorRegistry::<T>::iter()
                .filter(|(_, info)| info.status == ValidatorStatus::Active)
                .count();

            // For now, require 2 signatures for execution (2-of-3 model)
            ...
            // Update proposal
            let signature_count = proposal.signatures.len() as u32;
            ProposalRegistry::<T>::insert(proposal_id, proposal);

            // Emit event
            Self::deposit_event(Event::ProposalSigned {
                proposal_id,
                signer: who,
                signature_count,
            });
    ...
    impl<T: Config> Pallet<T> {
        fn execute_proposal(proposal: &ProposalDetails<T>) -> DispatchResult {
            ensure!(!proposal.executed, Error::<T>::NoPendingChanges);

            match proposal.action {
                ControlAction::Pause => Self::deposit_event(Event::SystemPaused {
                    initiated_by: proposal.created_by.clone(),
                }),
                ControlAction::Resume => Self::deposit_event(Event::SystemResumed {
                    initiated_by: proposal.created_by.clone(),
                }),
        ...
        fn do_execute_dual_tx(
            ...
            let _evm_receipt = evm_tx.map(|_tx| ExecutionReceipt {
                success: true,
                gas_used: 21000,
                return_data: BoundedVec::new(),
                logs: BoundedVec::new(),
                state_changes: BoundedVec::new(),
            });
