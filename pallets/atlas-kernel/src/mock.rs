...
impl pallet_atlas_kernel::EvmExecutionAdapter for MockEvmAdapter {
	fn validate_bytecode(&self, payload: &[u8]) -> Result<(), frame_support::dispatch::DispatchError> {
...
impl pallet_atlas_kernel::SvmExecutionAdapter for MockSvmAdapter {
	fn validate_program(&self, payload: &[u8]) -> Result<(), frame_support::dispatch::DispatchError> {
