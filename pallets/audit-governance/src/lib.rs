...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type AuditArtifacts<T: Config> =
        StorageMap<_, Blake2_128Concat, H256, AuditArtifact, OptionQuery>;

    /// Latest audit for a given commit (fast lookup)
    #[pallet::storage]
    pub type LatestAuditForCommit<T: Config> =
        StorageMap<_, Blake2_128Concat, H256, H256, OptionQuery>;

    /// Appeal tracking: audit_id → (deadline_block, appeal_votes)
    #[pallet::storage]
    pub type AuditAppeals<T: Config> =
        StorageMap<_, Blake2_128Concat, H256, (BlockNumberFor<T>, u32), OptionQuery>;

    /// Locked agents: account → Some(audit_id) if locked
    #[pallet::storage]
    pub type LockedAgents<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, Option<H256>, ValueQuery>;

    /// Whitelisted audit submitters
    #[pallet::storage]
    pub type AuditSubmitters<T: Config> =
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_audit_artifact(
            ...
            let bounded_findings =
                BoundedVec::try_from(artifact_findings).map_err(|_| Error::<T>::TooManyFindings)?;
        ...
        pub fn appeal_audit(origin: OriginFor<T>, audit_id: H256) -> DispatchResult {
            ...
            // Get audit
            let artifact = AuditArtifacts::<T>::get(audit_id).ok_or(Error::<T>::AuditNotFound)?;
        ...
        pub fn lock_agent(
            ...
            // Verify audit exists
                ...
                AuditArtifacts::<T>::contains_key(audit_id),
                Error::<T>::AuditNotFound
            );
        ...
        pub fn schedule_proposal(
            ...
            let current_block = frame_system::Pallet::<T>::block_number();
            let execution_block =
                current_block.defensive_saturating_add(T::GovernanceTimelock::get());
        ...
        pub fn execute_proposal(origin: OriginFor<T>, proposal_id: H256) -> DispatchResult {
            ensure_root(origin)?;

            // Get proposal and execution block
            let (_action_desc, execution_block) =
                PendingProposals::<T>::get(&proposal_id).ok_or(Error::<T>::ProposalNotFound)?;
